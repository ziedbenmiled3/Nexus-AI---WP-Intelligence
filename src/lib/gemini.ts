...
export const geminiQuery = async (payload: any, customKey?: string): Promise<GeminiResponse> => {
  try {
    ...
    // Normalize model selection to modern gemini-3.5-flash
    let finalModel = payload.model || "gemini-3.5-flash";
    if (finalModel === "gemini-3-flash-preview" || finalModel === "gemini-2.0-flash") {
      finalModel = "gemini-3.5-flash";
    }
    
    const payloadWithLang = { ...payload, model: finalModel, lang: currentLang };
