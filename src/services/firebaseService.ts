...
export const firebaseService = {
  ...
  async getSubscription(email: string) {
    ...
    try {
      ...
      let siteLimit = plan?.site_limit || plan?.sites || 0;
      let planName = plan?.name || 'Inconnu';

      try {
        const settings = await this.getSettings();
        if (settings && settings['nexus_matrix_config']) {
          const matrix = typeof settings['nexus_matrix_config'] === 'string' 
            ? JSON.parse(settings['nexus_matrix_config']) 
            : settings['nexus_matrix_config'];
          
          if (matrix && matrix.packs && matrix.packs[subData.plan_id]) {
            const mPack = matrix.packs[subData.plan_id];
            if (mPack.name) planName = mPack.name;
            if (typeof mPack.siteLimit === 'number') {
              siteLimit = mPack.siteLimit;
            } else if (typeof mPack.site_limit === 'number') {
              siteLimit = mPack.site_limit;
            }
          }
        }
      } catch (e) {
        console.error('Error fetching site_limit from matrix config:', e);
      }
    ...
    try {
      ...
      try {
        ...
        if (settings && settings['nexus_matrix_config']) {
          ...
          if (matrix && matrix.packs && matrix.packs[planId]) {
            isLifetime = !!matrix.packs[planId].isLifetime;
          }
        }
      } catch (e) {
        console.error('Error checking isLifetime in subscribe:', e);
      }

      if (isLifetime) {
        // Lifetime subscription expires in 100 years
        expiresAt.setFullYear(expiresAt.getFullYear() + 100);
      } else if (planId === 'trial') {
        const plans = await this.getPlans();
        const trialPlan = plans.find(p => p.id === 'trial');
        const durationHours = trialPlan?.duration_hours || 24;
  ...
  async clearAllPayments() {
    try {
      ...
      const promises = snapshot.docs.map(docSnap => deleteDoc(doc(db, 'payments', docSnap.id)));
      await Promise.all(promises);

      // Également effacer toutes les souscriptions actives pour réinitialiser l'état des abonnements de test
      const subSnapshot = await getDocs(collection(db, 'subscriptions'));
      const subPromises = subSnapshot.docs.map(docSnap => deleteDoc(doc(db, 'subscriptions', docSnap.id)));
      await Promise.all(subPromises);

      return { success: true };
    } catch (error) {
      handleFirestoreError(error, OperationType.DELETE, 'payments/subscriptions');
    }
  },
  ...
  async deleteUserAccount(uid: string, email: string) {
    try {
      const cleanEmail = email.toLowerCase().trim();
      
      // 1. Delete user doc in 'users' by UID if provided
      if (uid) {
        await deleteDoc(doc(db, 'users', uid));
      }
      
      // ALSO query by email to clean up any duplicates or mismatched/manual document IDs
      const userQ = query(collection(db, 'users'), where('email', '==', cleanEmail));
      const userSnap = await getDocs(userQ);
      const userDeletePromises = userSnap.docs.map(docSnap => deleteDoc(doc(db, 'users', docSnap.id)));
      await Promise.all(userDeletePromises);

      // 2. Delete subscription doc by UID/emailSlug/query
      if (uid) {
        await deleteDoc(doc(db, 'subscriptions', uid));
      }
      const emailSlug = cleanEmail.replace(/[@.]/g, '_');
      await deleteDoc(doc(db, 'subscriptions', emailSlug));
      
      const subQ = query(collection(db, 'subscriptions'), where('user_email', '==', cleanEmail));
      const subSnap = await getDocs(subQ);
      const subsDeletePromises = subSnap.docs.map(docSnap => deleteDoc(doc(db, 'subscriptions', docSnap.id)));
      await Promise.all(subsDeletePromises);

      // 3. Delete user's sites
      const sitesQuery = query(collection(db, 'sites'), where('user_email', '==', cleanEmail));
      const sitesSnap = await getDocs(sitesQuery);
      const deletePromises = sitesSnap.docs.map(docSnap => deleteDoc(doc(db, 'sites', docSnap.id)));
      await Promise.all(deletePromises);

      // Clean up site_registry records for this user
      const registryQuery = query(collection(db, 'site_registry'), where('user_email', '==', cleanEmail));
      const registrySnap = await getDocs(registryQuery);
      const registryDeletePromises = registrySnap.docs.map(docSnap => deleteDoc(doc(db, 'site_registry', docSnap.id)));
      await Promise.all(registryDeletePromises);

      // 4. Delete user from Firebase Auth via standard superadmin backend API
      try {
        ...
        await fetch('/api/admin/delete-user', {
          ...
          body: JSON.stringify({ uid, email: cleanEmail })
        });
      } catch (authError) {
        console.warn('[Firebase] Warning: Auth deletion API failed or bypassed:', authError);
      }
