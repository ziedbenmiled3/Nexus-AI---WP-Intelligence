    ...
    <div className="space-y-6 relative min-h-[600px]">
        ...
        {showSuccess && (
          ...
          >
            ...
            <div className="flex flex-col">
               <span className="text-sm font-black uppercase tracking-widest leading-none">{isEn ? 'Applied Intelligence' : 'Intelligence Appliquée'}</span>
               <span className="text-[9px] font-bold text-blue-200 uppercase tracking-widest mt-1">{isEn ? 'Keywords have been injected into your SEO database' : 'Les mots-clés ont été injectés dans votre base SEO'}</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] flex items-center gap-2">
               <Globe className="w-3 h-3" /> {isEn ? 'Competitor Scouting & Niche Strategy' : 'Scrutage concurrentiel & Strategie de niche'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 text-right">
           <div className="hidden lg:block">
              <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">{isEn ? 'Updated' : 'Mise à jour'}</p>
              <p className="text-[10px] font-black text-white uppercase tracking-widest">{isEn ? 'Real-time (Live)' : 'Temps réel (Live)'}</p>
           </div>
           <div className="w-px h-8 bg-slate-800 hidden lg:block" />
           <div className="text-right">
              <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">{isEn ? 'Analysis ID' : 'ID Analyse'}</p>
        ...
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4 relative z-10">
          <div className="flex-1 space-y-2">
            <div className="flex items-center justify-between px-1">
              <label className="text-[10px] font-bold uppercase text-slate-500 tracking-widest leading-none">{isEn ? 'Target Niche or Product' : 'Niche ou Produit cible'}</label>
              <div className="relative">
                ...
                >
                  ...
                  <span>{isEn ? 'Choose a category' : 'Choisir une catégorie'}</span> 
                        ...
                        <div className="max-h-80 overflow-y-auto custom-scrollbar">
                          {isLoadingCats ? (
                            <div className="p-8 text-center text-slate-600">
                              ...
                              <p className="text-[9px] font-bold uppercase tracking-widest">{isEn ? 'Initializing...' : 'Initialisation...'}</p>
                            </div>
                            ...
                            <div className="p-8 text-center text-[9px] text-slate-500 font-bold uppercase tracking-widest leading-relaxed">
                              {isEn ? <>No segmented<br/>categories found</> : <>Aucune catégorie<br/>segmentée trouvée</>}
                            </div>
                          )}
                        </div>
                      </motion.div>
            ...
            <div className="relative group">
              ...
              <input 
                type="text" 
                placeholder={isEn ? "e.g., Luxury sneakers, Organic cosmetics..." : "Ex: Sneakers de luxe, Cosmétiques bio..."}
          ...
          <div className="w-full md:w-56 space-y-2">
            <label className="text-[10px] font-bold uppercase text-slate-500 px-1 tracking-widest leading-none">{isEn ? 'Market (Country)' : 'Marché (Pays)'}</label>
            <div className="relative group">
               ...
               <input 
                type="text" 
                placeholder={isEn ? "United States" : "France"}
          ...
          <div className="flex items-end">
            ...
            >
              {isSearching ? <Loader2 className="w-5 h-5 animate-spin" /> : <>{isEn ? 'Analyze' : 'Analyser'} <ArrowRight className="w-4 h-4" /></>}
            </button>
          </div>
        </form>
      </div>
       ...
       {isSearching && (
        <div className="bg-slate-900 rounded-3xl p-24 text-center border border-slate-800 relative overflow-hidden">
           ...
           <div className="relative z-10">
              ...
              <p className="text-xl font-bold tracking-tight text-white mb-2 italic font-serif">
                {isEn ? 'The Maestro is analyzing the global market...' : 'Le Maestro analyse le marché mondial...'}
              </p>
              <p className="text-slate-500 text-sm font-medium">
                {isEn ? 'Extracting competitor data in progress.' : 'Extraction des données concurrentielles en cours.'}
              </p>
           </div>
        </div>
      )}
        ...
        {trendData.length > 0 && !isSearching && (
          ...
          >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-sm font-black text-white uppercase italic tracking-tighter">
                      {isEn ? 'Trend Analysis (6 Months)' : 'Analyse de Tendance (6 mois)'}
                    </h3>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-none">
                      {isEn ? 'Search interest over time' : 'Intérêt de recherche dans le temps'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 flex flex-col justify-center relative overflow-hidden group">
               ...
               <div className="relative z-10">
                  <h4 className="text-[10px] font-black text-blue-500 uppercase tracking-[0.2em] mb-4">
                    {isEn ? 'Market Summary' : 'Résumé du Marché'}
                  </h4>
                  <p className="text-sm font-bold text-slate-300 leading-relaxed italic">
                    {marketSummary || (isEn ? "The market shows signs of stable growth with a moderate concentration of direct competitors." : "Le marché affiche des signes de croissance stable avec une concentration modérée de concurrents directs.")}
                  </p>
               </div>
               <div className="mt-8 pt-8 border-t border-slate-800/50 flex items-center justify-between">
                  <div>
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">
                      {isEn ? 'Niche Potential' : 'Potentiel Niche'}
                    </p>
          ...
          >
            ...
            <div className="xl:col-span-2 bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 relative overflow-hidden group">
               ...
               <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                     <h3 className="text-sm font-black text-white uppercase italic tracking-tighter">
                       {isEn ? 'Positioning Radar' : 'Radar de Positionnement'}
                     </h3>
                     <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-none">
                       {isEn ? '360° Multi-Engine Comparison' : 'Comparaison multi-moteurs 360°'}
                     </p>
                  </div>
               </div>
               ...
               <div className="h-[350px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                     <RadarChart cx="50%" cy="50%" outerRadius="80%" data={[
                        ...
                        { subject: isEn ? 'Alternative Algos' : 'Algos Alternatifs', A: 65, B: results[0]?.visibilityScores.others || 0, C: results[1]?.visibilityScores.others || 0, fullMark: 100 },
                        ...
                        <RadarArea name={isEn ? 'Your Site' : 'Votre Site'} dataKey="A" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.2} strokeWidth={3} />
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 flex flex-col group">
               <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                     <h3 className="text-sm font-black text-white uppercase italic tracking-tighter">
                       {isEn ? 'Visibility Benchmark' : 'Benchmark Visibilité'}
                     </h3>
                     <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-none">
                       {isEn ? 'Global Presence Score' : 'Score Global de Présence'}
                     </p>
                  </div>
               </div>
               ...
               <div className="flex-1 w-full min-h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                     <BarChart 
                        layout="vertical" 
                        data={[
                           { name: isEn ? 'YOU' : 'VOUS', score: 58, fill: '#3b82f6' },
                           ...results.map((r, i) => ({ 
              ...
              <div className="py-20 text-center space-y-6">
                ...
                <div>
                   <h3 className="text-2xl font-black text-white uppercase italic tracking-tighter mb-2">
                     {isEn ? 'Calculating Semantic Gap...' : 'Calcul du Semantic Gap...'}
                   </h3>
                   <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest italic">
                     {isEn ? "Comparing your structure with the market leader's" : 'Comparaison de votre structure avec le leader du marché'}
                   </p>
                </div>
              </div>
            ) : gapAnalysis && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                  <div className="space-y-8">
                     <div>
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-6 flex items-center gap-3">
                           ...
                           {isEn ? 'Missing Opportunities (Gap)' : 'Opportunités Manquantes (Gap)'}
                        </p>
                          ...
                          {gapAnalysis.missingOpportunities.map((op: string, i: number) => (
                            <div key={i} className="group relative">
                              ...
                              <button 
                                ...
                                title={isEn ? 'Generate ad campaign' : 'Générer campagne publicitaire'}
                              >
                        ...
                        {isGeneratingAd && (
                          <div className="flex items-center gap-3 text-blue-400 italic mb-6">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">
                              {isEn ? 'Generating ad campaign...' : 'Génération de la campagne IA...'}
                            </span>
                          </div>
                        )}
                     </div>
                     ...
                     <div>
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-6 flex items-center gap-3">
                           ...
                           {isEn ? 'Semantic Overlap (Strengths)' : 'Chevauchement Sémantique (Forces)'}
                        </p>
                  ...
                  <div className="bg-slate-950/50 border border-white/5 rounded-3xl p-8 space-y-6">
                     <div className="flex items-center gap-4 border-b border-white/5 pb-6">
                        ...
                        <div>
                           <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest">{isEn ? 'Nexus Roadmap' : 'Feuille de Route Nexus'}</p>
                           <p className="text-sm font-black text-white uppercase tracking-tight italic">{isEn ? 'Strategic Guidelines' : 'Directives Stratégiques'}</p>
                        </div>
                     </div>
                     ...
                     <div className="pt-4">
                        ...
                        >
                          {isInjecting === -1 ? (
                             <>
                               <Loader2 className="w-4 h-4 animate-spin" />
                               {isEn ? 'Strategic Injecting...' : 'Injection Stratégique...'}
                             </>
                          ) : (
                             <>
                               <Zap className="w-4 h-4" /> {isEn ? 'Bridge Semantic Gaps' : 'Combler les Lacunes Sémantiques'}
                             </>
                          )}
                        </button>
                     </div>
               ...
               >
                  {isInjecting === idx ? (
                    <>
                      ...
                      {isEn ? 'Injecting...' : 'Injection...'}
                    </>
                  ) : (
                    <>{isEn ? 'Inject keywords' : 'Injecter les mots clés'}</>
                  )}
               </button>
            </div>
          </motion.div>
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 lg:p-12">
            ...
            >
              <div className="p-8 border-b border-white/5 flex items-center justify-between sticky top-0 bg-[#0a0c10]/80 backdrop-blur-xl z-10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-none">
                      {isEn ? 'AI-generated ad campaign' : 'Campagne publicitaire générée par IA'}
                    </p>
                  </div>
                </div>
                <button 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="p-6 bg-slate-950 border border-slate-800 rounded-3xl space-y-4">
                      ...
                      <div>
                        <p className="text-[9px] text-blue-500 font-black uppercase tracking-widest mb-2">
                          {isEn ? 'Suggested Keywords' : 'Mots-clés suggérés'}
                        </p>
                        <div className="flex flex-wrap gap-2">
                ...
                <div className="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-8 pt-12 border-t border-white/5">
                  <div className="lg:col-span-1 space-y-4">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                      <Target className="w-4 h-4" /> {isEn ? 'Target Audience' : 'Audience Cible'}
                    </p>
                    <div className="flex flex-wrap gap-2">
                  ...
                  <div className="lg:col-span-2 space-y-4">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                      <Zap className="w-4 h-4" /> {isEn ? 'Nexus Strategy' : 'Stratégie Nexus'}
                    </p>
              ...
              <div className="p-8 border-t border-white/5 bg-slate-950/50">
                ...
                >
                  {isEn ? 'Close & Apply Strategy' : 'Fermer & Appliquer la Stratégie'}
                </button>
              </div>
            </motion.div>
          </div>
      ...
      {!isSearching && results.length === 0 && (
        <div className="py-24 text-center bg-slate-900/20 border border-dashed border-slate-800 rounded-[3rem] relative overflow-hidden group">
           ...
           <h3 className="text-xl font-bold text-white uppercase tracking-tighter italic mb-3 relative z-10">
             {isEn ? 'Target Not Locked' : 'Cible Non Verrouillée'}
           </h3>
           <p className="text-slate-500 font-bold uppercase tracking-[0.15em] text-[10px] max-w-xs mx-auto leading-relaxed relative z-10">
              {isEn ? 'Enter a niche and country to launch the scan of market leaders.' : 'Entrez une niche et un pays pour lancer le scan des leaders du marché.'}
           </p>
