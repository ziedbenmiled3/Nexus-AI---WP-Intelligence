...
const CustomNexusTooltip = ({ active, payload, label, title, color, isEn }: any) => {
  if (active && payload && payload.length) {
    const value = payload[0].value;
    
    let insight = "";
    if (title === "Évolution SEO" || title === "SEO Evolution") {
      insight = value > 85 
        ? (isEn ? "Nexus-Search: Optimal positioning. Stable organic growth." : "Nexus-Search : Positionnement optimal. Croissance organique stable.")
        : (isEn ? "Nexus-Search: Progressive leverage. Main target keywords detected." : "Nexus-Search : Amélioration progressive. Opportunités de mots-clés détectées.");
    } else if (title === "Santé Globale" || title === "Global Health") {
      insight = value > 92 
        ? (isEn ? "Optimal system. Fast local response speeds (ms)." : "Système optimal. Temps de réponse chirurgical (ms).") 
        : (isEn ? "Fluctuating stability. Optimizing local SQL database requests recommended." : "Stabilité fluctuante. Optimisation des requêtes SQL recommandée.");
    } else if (title === "Flux Stocks" || title === "Inventory Flows") {
      insight = value >= 70 
        ? (isEn ? "Optimized flows. Catalog rotatability perfectly aligned with seasonal demand." : "Flux optimisé. Rotation d'inventaire en accord avec la saisonnalité.") 
        : (value > 40 
          ? (isEn ? "Vigilance region. Nexus recommends monitoring low-stock items." : "Zone de vigilance. Nexus recommande de surveiller les stocks critiques.") 
          : (isEn ? "Low stock warnings: Urgent supply needed to keep active MRR." : "Alerte Rupture : Ravitaillement urgent requis pour maintenir le CA."));
    }
      ...
      <div className="bg-[#0a0c10] border border-slate-800 p-4 rounded-2xl shadow-2xl backdrop-blur-md animate-in fade-in zoom-in duration-300 min-w-[200px] pointer-events-none">
        ...
        <div className="flex items-baseline gap-2 mb-3">
            ...
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
              {isEn ? "Nexus Points" : "Points Nexus"}
            </span>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
         <div>
            <h1 className="text-5xl font-black text-white italic uppercase tracking-tighter mb-3">
              {isEn ? "System Status" : "Statut Système"}
            </h1>
            <div className="flex items-center gap-3">
               ...
               <span className="text-xs font-black text-slate-500 uppercase tracking-[0.4em] whitespace-nowrap">
                  {isEn ? "Live Telemetry: " : "Télémétrie Live : "}<span className="text-slate-400">{config.url ? config.url.replace('https://', '').replace(/\/$/, '') : (isEn ? "NO ACTIVE SITE" : "AUCUN SITE ACTIF")}</span>
               </span>
            </div>
         </div>
         <button 
         ...
         >
            ...
            {isEn ? "Full Audit Log" : "Journal d'Audit Complet"}
         </button>
      </div>
               ...
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <h3 className="text-sm font-black text-white uppercase italic tracking-tighter">
                       {isEn ? "SEO Evolution" : "Évolution SEO"}
                     </h3>
                     <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">
                       {isEn ? "Nexus-Search Score (30d)" : "Score Nexus-Search (30j)"}
                     </p>
                  </div>
               </div>
            </div>
            ...
            <div className="h-[200px] w-full mt-auto relative z-10">
               <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={trendData}>
                     ...
                     <Tooltip 
                        content={<CustomNexusTooltip title="Évolution SEO" color="bg-blue-500" isEn={isEn} />}
         ...
         <div className="bg-[#0a0c10] border border-slate-800/80 rounded-[2.5rem] p-8 flex flex-col group hover:border-emerald-500/30 transition-all relative overflow-hidden h-[340px]">
            <div className="flex justify-between items-center mb-8 relative z-10">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <h3 className="text-sm font-black text-white uppercase italic tracking-tighter">
                       {isEn ? "Global Health" : "Santé Globale"}
                     </h3>
                     <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                       {isEn ? "Infrastructure Stability (30d)" : "Stabilité Infrastructure (30j)"}
                     </p>
                  </div>
               </div>
            </div>
            ...
            <div className="h-[200px] w-full mt-auto relative z-10">
               <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={trendData}>
                     ...
                     <YAxis hide domain={[0, 100]} />
                     <Tooltip 
                        content={<CustomNexusTooltip title="Santé Globale" color="bg-emerald-500" isEn={isEn} />}
                        cursor={{ stroke: '#334155', strokeWidth: 1 }}
                     />
                     <Area 
                        type="monotone" 
         ...
         <div className="bg-[#0a0c10] border border-slate-800/80 rounded-[2.5rem] p-8 flex flex-col group hover:border-orange-500/30 transition-all relative overflow-hidden h-[340px]">
            <div className="flex justify-between items-center mb-8 relative z-10">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <h3 className="text-sm font-black text-white uppercase italic tracking-tighter">
                       {isEn ? "Inventory Flows" : "Flux Stocks"}
                     </h3>
                     <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                       {isEn ? "Inventory Rotation (30d)" : "Rotation Inventaire (30j)"}
                     </p>
                  </div>
               </div>
            </div>
               ...
               <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={trendData}>
                     ...
                     <Tooltip 
                        ...
                        content={<CustomNexusTooltip title="Flux Stocks" color="bg-orange-500" isEn={isEn} />}
                     />
         ...
         <div className="relative z-10 space-y-6">
            ...
            <div className="max-w-3xl">
               <h3 className="text-2xl font-black text-white uppercase tracking-tight mb-4">
                  {isEn ? "Store Strategic Report" : "Rapport Stratégique de la Boutique"}
               </h3>
               <p className="text-slate-400 text-sm font-medium leading-relaxed mb-8">
                  {hasData 
                    ? (isEn 
                      ? `Nexus is actively analyzing your ${stats.products} products and ${stats.posts} articles to detect exponential growth vectors.`
                    ...
                    : (isEn
                      ? "Connect your store so Nexus can process products and articles to deliver custom strategic reports."
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="p-6 rounded-3xl bg-white/5 border border-white/5 flex flex-col gap-3">
                     <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">
                       {isEn ? "Priority Focus" : "Focus Prioritaire"}
                     </span>
                     <p className="text-xs font-bold text-slate-200">
                        {!hasData 
                          ? (isEn ? "Awaiting data for deep SEO analysis." : "En attente de données pour l'analyse SEO.")
                          : stats.seoScore < 85 
                            ? (isEn ? "SEO Optimization required for 12 high-volume search parameters." : "Optimisation SEO requise pour 12 mots-clés à fort volume.") 
                            : (isEn ? "Maintaining SEO superiority: 92% of KPIs in green status." : "Maintien de la dominance SEO : 92% des KPIs au vert.")}
                     </p>
                  </div>
                  <div className="p-6 rounded-3xl bg-white/5 border border-white/5 flex flex-col gap-3">
                     <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">
                       {isEn ? "Revenue Health" : "Santé des Revenus"}
                     </span>
                     <p className="text-xs font-bold text-slate-200">
                        {hasData 
                          ? (isEn ? "Potential increase of 15% average order value via custom AI bundles." : "Potentiel d'augmentation de 15% du panier moyen via bundles IA.")
                          : (isEn ? "Revenue analysis currently not available." : "Analyse des revenus non disponible.")}
                     </p>
                  </div>
               </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="bg-slate-900/40 border border-slate-800 p-8 rounded-[2.5rem] space-y-6">
            ...
            <div className="space-y-3">
                {[
                   ...
                   const details = [
                      {
                        title: isEn ? "Critical Issues" : "Problèmes Critiques",
                        subtitle: isEn ? "Urgent actions required" : "Corrections urgentes requises",
                        color: "text-red-500",
                        bg: "bg-slate-950/95",
                        border: "border-red-500/30 shadow-red-950/50",
                        badgeColor: "bg-red-500/10 text-red-400 border-red-500/20",
                        items: [
                          { code: "IMG-ERR", text: isEn ? "4 product pictures are missing or corrupted" : "4 fiches produits ont des images manquantes ou corrompues" },
                          { code: "VAR-0", text: isEn ? "3 variations have no general or sale price configured" : "3 variations n'ont pas de prix général ou promo défini" },
                          { code: "SEO-404", text: isEn ? "2 critical landing pages lead to a 404 error" : "2 pages de redirection SEO clés renvoient vers une erreur 404" },
                          { code: "ALT-TAG", text: isEn ? "3 essential visuals are missing descriptive Alt tag" : "3 visuels produits essentiels n'ont pas d'attribut Alt" }
                        ]
                      },
                      {
                        title: isEn ? "Warnings" : "Avertissements",
                        subtitle: isEn ? "Recommended enhancements" : "Améliorations recommandées",
                        color: "text-amber-500",
                        bg: "bg-slate-950/95",
                        border: "border-amber-500/30 shadow-amber-950/50",
                        badgeColor: "bg-amber-500/10 text-amber-400 border-amber-500/20",
                        items: [
                          { code: "DESC-SH", text: isEn ? "14 short product descriptions are empty or too short" : "14 descriptions courtes de produits sont absentes ou vides" },
                          { code: "META-0", text: isEn ? "6 product categories missing SEO description" : "6 catégories de produits n'ont pas de méta-description" },
                          { code: "STK-LOW", text: isEn ? "4 high-performance items logic-triggered low stock" : "4 articles à fort trafic approchent de la limite de stock bas" }
                        ]
                      },
                      {
                        title: isEn ? "Optimizations Verified" : "Éléments Optimisés",
                        subtitle: isEn ? "Performance & SEO in success" : "Succès SEO et performances au vert",
                        color: "text-green-500",
                        bg: "bg-slate-950/95",
                        border: "border-green-500/30 shadow-green-950/50",
                        badgeColor: "bg-green-500/10 text-green-400 border-green-500/20",
                        items: [
                          { code: "TAG-HX", text: isEn ? "Perfect Hx heading hierarchy verified" : "Structure des balises Hx valide et hiérarchisée" },
                          { code: "CACHE", text: isEn ? "WooCommerce template transient cache cleaned" : "Cache temporaire et transients WooCommerce purgés" },
                          { code: "FEED-OK", text: isEn ? "Google Merchant feed active & indexed (100%)" : "Flux Google Shopping actif et synchronisé à 100%" }
                        ]
                      }
                   ][i];
                   ...
                   return (
                      ...
                      >
                         <div className={cn("flex items-center justify-between p-4 rounded-2xl border transition-all hover:scale-[1.02] cursor-help", item.bg, item.border)}>
                            <div className="flex items-center gap-4">
                         ...
                         <AnimatePresence>
                            {hoveredAlert === i && !stats.loading && (
                               <motion.div
                                  ...
                                  className={cn(
                                     "absolute z-50 p-5 rounded-3xl border shadow-2xl w-80 backdrop-blur-md pointer-events-none text-left transition-all",
                                     details.bg, details.border,
                                     "lg:left-full lg:top-0 lg:ml-4 lg:-translate-y-0",
                                     "left-0 right-0 top-full mt-2 lg:mt-0"
                                  )}
                               >
                                  <div className="mb-2">
                                     <h4 className={cn("text-xs font-black uppercase tracking-widest", details.color)}>
                                        {details.title}
                                     </h4>
                                  ...
                                  <div className="w-full h-px bg-slate-800/80 my-2.5" />
                                  <div className="space-y-2.5">
                                     {details.items.map((sub, sIdx) => (
                                        <div key={sIdx} className="flex gap-2.5 items-start">
                                           <span className={cn("text-[7px] font-mono px-1.5 py-0.5 rounded border uppercase tracking-wider h-max mt-0.5 shrink-0 font-bold", details.badgeColor)}>
                                              {sub.code}
                                           </span>
         ...
         <div className="bg-slate-900/40 border border-slate-800 p-8 rounded-[2.5rem]">
            ...
            <div className="space-y-2">
               {[
                  { id: 'social', label: isEn ? "Nexus Video & Voice Studio" : "Studio Vidéo & Voix IA", desc: isEn ? "Horizontal & vertical AIDA marketing videos" : "Vidéos de vente AIDA & voix off réalistes", icon: Share2, highlight: true },
