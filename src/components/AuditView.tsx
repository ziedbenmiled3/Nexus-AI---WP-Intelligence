...
export default function AuditView({ config }: { config: WPConfig }) {
  ...
  const applySelected = async () => {
    ...
    setIsAuditing(true);
    setAuditStep('Application des corrections...');
    try {
  ...
  const handleAudit = async () => {
    ...
    setResults(null);
    try {
      setAuditStep('Indexation de la structure du site...');
      await new Promise(r => setTimeout(r, 1000));
      
      setAuditStep('Analyse SEO des articles (Posts)...');
      const posts = await getPosts(config, { per_page: 5, status: 'publish' });
      
      setAuditStep('Analyse Marketing des produits...');
      const products = await getProducts(config, { per_page: 5, status: 'publish' });

      setAuditStep('Exécution de l\'IA Maestro Audit...');
      ...
      console.error(err);
      setAuditStep(`Erreur critique IA: ${err.message || JSON.stringify(err)}`);
  ...
  return (
    <div className="space-y-6 relative">
      <AnimatePresence>
        {showSuccess && (
          ...
          >
            ...
            <span className="text-sm font-black uppercase tracking-widest">Stratégie appliquée avec succès</span>
          </motion.div>
        )}
      </AnimatePresence>
      {/* Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: 'Articles', val: stats.posts, icon: Activity, color: 'text-blue-400', bg: 'bg-blue-400/10' },
          { label: 'Produits', val: stats.products, icon: Zap, color: 'text-amber-400', bg: 'bg-amber-400/10' },
          { label: 'Score SEO Global', val: results?.score ? `${results.score}%` : '--', icon: BarChart3, color: 'text-emerald-400', bg: 'bg-emerald-400/10' },
        ].map((item, idx) => (
      ...
      {!results && !isAuditing && (
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-12 text-center text-white relative overflow-hidden group">
          ...
          <h2 className="text-2xl font-bold mb-2 tracking-tight">Audit Global Recommandé</h2>
      ...
      {isAuditing && (
        <div className="bg-slate-900 rounded-3xl p-20 text-center border border-slate-800">
          ...
          <h2 className="text-xl font-bold mb-2 tracking-tight text-white">{auditStep || "Analyse de la Structure..."}</h2>
          <p className="text-slate-500 text-sm font-medium">Le Maestro scanne vos métadonnées et analyse la sémantique.</p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-bold text-white tracking-tight">Focus SEO</h3>
              </div>
            </div>
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-bold text-white tracking-tight">Qualité du Contenu</h3>
              </div>
            </div>
            <div className="space-y-3 flex-1">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-lg font-bold text-white tracking-tight">Optimisation des Titres (Nexus)</h3>
            </div>
            <div className="p-6 bg-slate-950 border border-slate-800 rounded-xl relative group">
               ...
               <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Titre suggéré pour l'élément analysé :</p>
               <h4 className="text-xl font-bold text-white italic tracking-tight mb-4">
                 "{results.optimizedTitle}"
               </h4>
               <button 
                  onClick={() => toggleSuggestion(`Appliquer le titre optimisé : ${results.optimizedTitle}`)}
                  className={`px-6 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${
                    selectedSuggestions.some(s => s.startsWith('Appliquer le titre optimisé'))
                      ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/20'
                      : 'bg-slate-800 text-slate-400 hover:bg-slate-700'
                  }`}
               >
                 {selectedSuggestions.some(s => s.startsWith('Appliquer le titre optimisé')) ? 'Titre sélectionné' : 'Sélectionner ce titre'}
               </button>
            </div>
          </motion.div>
          ...
          <div className="md:col-span-2 space-y-4">
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-1 shadow-xl shadow-blue-900/20">
               <div className="bg-slate-950 p-6 rounded-[calc(1rem-1px)] flex items-center justify-between text-white border border-white/5">
                  <div>
                     <p className="text-[10px] font-bold text-blue-400 uppercase tracking-[0.2em] mb-1">Résumé Global Santé</p>
            ...
            {selectedSuggestions.length > 0 && (
              ...
              >
                <ShieldCheck className="w-4 h-4" /> Appliquer les {selectedSuggestions.length} recommandations sélectionnés
              </motion.button>
            )}
          </div>
        </div>
