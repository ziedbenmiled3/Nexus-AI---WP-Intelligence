...
export default function SitesView({ 
  ...
  const isAdminUser = user?.email?.toLowerCase() === 'ziedbenmiled3@gmail.com';
  const siteLimit = currentSub?.site_limit || (isAdminUser ? 100 : 0);
  ...
  const fetchSites = useCallback(async (showLoader = false) => {
    ...
    try {
      const data = await firebaseService.getSites(user.email);
      if (Array.isArray(data)) {
        ...
        if (isAdminUser) {
          firebaseService.syncAllSitesToRegistry(user.email);
        }
      }
    } catch (err) {
...
RewriteRule ^(.*) - [E=HTTP_AUTHORIZATION:%1]`}
  ...
  const handleAddSite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If it's the first site, auto-connect
      if (sites.length === 0) onSwitch(config);
            ...
            {sites.map(site => (
              ...
              >
                ...
                <button 
                   ...
                   className={cn(
                     "w-full py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 transition-all active:scale-[0.98]",
                     isActive(site.url)
                       ? "bg-blue-600 text-white shadow-xl shadow-blue-900/40"
                       : "bg-slate-900 text-slate-500 border border-slate-800 hover:text-white"
                   )}
                >
                   {isActive(site.url) ? 'Tableau de bord' : 'Sélectionner le site'}
