...
export default function ComparisonTable({ config, selectedCurrency = 'EUR' }: ComparisonTableProps) {
    ...
    return (
      <div className="p-10 border border-red-500/20 bg-red-500/5 rounded-3xl text-center">
        <p className="text-red-500 font-black uppercase tracking-widest text-[10px]">
          {isEn ? 'Fragmented Configuration Matrix - Protocol Error' : 'Configuration Matrix Fragmentée - Erreur de Protocole'}
        </p>
      </div>
    );
  }
    ...
    <div className="w-full max-w-7xl mx-auto px-4 py-20">
      <div className="text-center mb-16">
        <h2 className="text-4xl font-display font-black text-white italic uppercase tracking-tighter mb-4">
          {isEn ? 'Nexus Matrix Comparison' : 'Comparatif Nexus Matrix'}
        </h2>
        <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em]">
          {isEn ? 'Architecture of privileges per SaaS protocol' : 'Architecture des privilèges par protocole SaaS'}
        </p>
      </div>
                    ...
                    <th key={packId} className="p-8 text-center min-w-[200px]">
                      <div className="space-y-1">
                        ...
                        <div className="text-[10px] font-bold text-purple-500 uppercase tracking-widest">
                          {(() => {
                            const parsedPrice = parseInt(pack.price.replace(/[^0-9]/g, '')) || 0;
                            if (parsedPrice === 0) {
                              return selectedCurrency === 'EUR' ? '0 €' : '0 $';
                            }
                            if (selectedCurrency === 'USD') {
                              return `${Math.round(parsedPrice * 1.08)} $`;
                            }
                            return `${parsedPrice} €`;
                          })()}
                        </div>
