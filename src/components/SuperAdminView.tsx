  ...
  const handleSavePlan = async (e: React.FormEvent) => {
    ...
    try {
      await firebaseService.updatePlan(editingPlan.id, {
        ...
        duration_hours: editingPlan.duration_hours ? Number(editingPlan.duration_hours) : (editingPlan.id === 'trial' ? 1 : null)
      });
      setEditingPlan(null);
      await loadData();
    } catch (err) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <div className="space-y-1 mt-1">
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.4em]">Superuser: {user?.email}</p>
            <p className="text-[8px] text-slate-600 font-mono uppercase tracking-[0.2em]">Nexus ID: {user?.uid}</p>
          </div>
        </div>
        ...
        <div className="bg-[#0c0e14] border border-slate-800 rounded-[2.5rem] p-10 relative overflow-hidden group">
          <div className="flex items-center justify-between mb-10 relative z-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-black text-white uppercase tracking-tighter italic">PROTOCOLE D'INSTALLATION PAYPAL</h3>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Passerelle de paiement globale Nexus</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6 relative z-10">
            <div className="p-6 bg-blue-600/5 border border-blue-500/10 rounded-2xl space-y-4">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] font-black text-white uppercase tracking-widest">Configuration Requise</p>
              </div>
              <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-loose">
                1. Assurez-vous d'avoir un compte <span className="text-white">PayPal BUSINESS</span>.<br/>
                2. Liez votre <span className="text-blue-400">Client ID</span> ci-dessous.
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest ml-1">PAYPAL CLIENT ID</label>
              <div className="flex gap-4">
                ...
                <button 
                  ...
                  className={cn(
                    "px-6 rounded-xl transition-all flex items-center justify-center gap-2",
                    paypalStatus === 'success' ? "bg-emerald-600 text-white" : "bg-blue-600 text-white hover:bg-blue-500"
                  )}
                >
                  {isSavingPaypal ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
                  <span className="text-[9px] font-black uppercase tracking-widest">{paypalStatus === 'success' ? 'OK' : 'SAVE'}</span>
                </button>
              </div>
            </div>
          </div>
                      ...
                      {String(editingPlan.id).toLowerCase().includes('trial') && (
                        <div className="p-8 bg-blue-600/10 border-2 border-blue-500 rounded-3xl space-y-4 shadow-[0_0_50px_rgba(37,99,235,0.2)]">
                          ...
                          <div className="space-y-3">
                            <label className="text-[10px] font-black uppercase text-white tracking-widest">DURÉE DE VALIDITÉ (EN HEURES)</label>
                            <div className="relative">
                              <input 
                                ...
                                required
                              />
                              <div className="absolute right-6 top-1/2 -translate-y-1/2 text-xl font-black text-blue-500 italic">HRS</div>
                            </div>
                            <div className="flex gap-4">
                               <div className="text-[9px] font-black text-slate-500 uppercase bg-white/5 px-3 py-2 rounded-lg border border-white/5">0.5 = 30m</div>
                               <div className="text-[9px] font-black text-slate-500 uppercase bg-white/5 px-3 py-2 rounded-lg border border-white/5">1 = 1h</div>
                               <div className="text-[9px] font-black text-slate-500 uppercase bg-white/5 px-3 py-2 rounded-lg border border-white/5">24 = 1 JOUR</div>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center justify-end gap-2">
                           <button 
                             ...
                             className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500 text-amber-500 hover:text-white border border-amber-500/20 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all"
                           ...
                           <button 
                             ...
                             className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-500/10 hover:bg-blue-500 text-blue-500 hover:text-white border border-blue-500/20 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all"
