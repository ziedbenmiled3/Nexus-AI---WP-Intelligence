...
export default function ProductManagerView({ config }: Props) {
  ...
  const renderWithButton = (content: any): any => {
    ...
    const regex = /\[VALIDATE:(\d+):([\d.]+)(?::(\d+))?\]/;
    const match = content.match(regex);
    
    if (match) {
      const productId = parseInt(match[1]);
      const salePrice = parseFloat(match[2]);
      const dayOffset = match[3] ? parseInt(match[3]) : 1;
                           ...
                           {product.on_sale && product.sale_price ? (
                              <div className="inline-flex flex-col items-center gap-0.5 text-amber-400 font-black">
                                 <div className="flex items-center gap-1 text-xs">
                                    ...
                                    {Math.round(((parseFloat(product.regular_price || product.price || "0") - parseFloat(product.sale_price)) / parseFloat(product.regular_price || product.price || "1")) * 100)}%
                                 </div>
