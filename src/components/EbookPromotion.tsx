...
import { ExternalLink, BookOpen, Sparkles, ArrowRight, Languages } from 'lucide-react';
...
export const EbookPromotion = ({ variant = 'sidebar' }: { variant?: 'sidebar' | 'dashboard' }) => {
  ...
  return (
    <motion.div 
      ...
      className="relative group overflow-hidden bg-[#0a0c10] border border-blue-500/20 rounded-[2rem] p-5 shadow-2xl mt-4"
    >
      {/* Glow Effect */}
      ...
      <div className="relative z-10 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex p-0.5 bg-black/60 rounded-lg border border-slate-800">
            <button 
              onClick={() => setSelectedLang('fr')}
              className={`px-1.5 py-0.5 rounded text-[8px] font-bold transition-all ${selectedLang === 'fr' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}
            >
              FR
            </button>
            <button 
              onClick={() => setSelectedLang('en')}
              className={`px-1.5 py-0.5 rounded text-[8px] font-bold transition-all ${selectedLang === 'en' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}
            >
              EN
            </button>
          </div>
