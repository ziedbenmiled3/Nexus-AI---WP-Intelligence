...
const TRANSLATIONS = {
  fr: {
    welcome: "Bonjour ! Je suis Nexus, votre assistant d'élite. Comment puis-je vous aider à propulser votre boutique WooCommerce aujourd'hui ?",
    activeProtocol: "PROTOCOLE ACTIF",
    thinking: "Le protocole Nexus réfléchit...",
    placeholder: "Posez votre question au protocole...",
    errorFallback: "Je n'ai pas pu générer de réponse.",
    errorConnection: "Erreur de connexion avec le protocole Nexus. Veuillez réessayer.",
    systemInstruction: "Tu es l'assistant d'élite de WP_AGENT.AI, un protocole d'IA révolutionnaire pour WooCommerce. Réponds en français de manière hyper qualifiée et toujours de façon très aidante."
  },
  en: {
    welcome: "Hello! I am Nexus, your elite assistant. How can I help you power up your WooCommerce store today?",
    activeProtocol: "ACTIVE PROTOCOL",
    thinking: "The Nexus protocol is thinking...",
    placeholder: "Ask the protocol your question...",
    errorFallback: "I could not generate a response.",
    errorConnection: "Connection error with the Nexus protocol. Please try again.",
    systemInstruction: "You are the elite assistant of WP_AGENT.AI, a revolutionary AI protocol for WooCommerce. You MUST respond in English with highly detailed, expert and helpful guidance."
  }
};
...
export default function AIChatSupport() {
  ...
  const tLocal = (key: keyof typeof TRANSLATIONS['fr']) => TRANSLATIONS[currentLang][key];
  ...
  // Initialize and update the greeting on language toggle (if no user messages are sent yet)
  useEffect(() => {
    if (messages.length === 0 || (messages.length === 1 && messages[0].role === 'model')) {
      setMessages([
        { role: 'model', text: TRANSLATIONS[currentLang].welcome }
      ]);
    }
  }, [currentLang]);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const res = await geminiQuery({
        ...
        systemInstruction: tLocal('systemInstruction')
      });
      
      setMessages(prev => [...prev, { role: 'model', text: res.data.text || tLocal('errorFallback') }]);
    } catch (error) {
      setMessages(prev => [...prev, { role: 'model', text: tLocal('errorConnection') }]);
    } finally {
      setIsLoading(false);
    }
  };
