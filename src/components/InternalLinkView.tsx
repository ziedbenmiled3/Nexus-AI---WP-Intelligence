...
export default function InternalLinkView({ config }: { config: WPConfig }) {
  ...
  const analyzeLinks = async (item: ContentItem) => {
    ...
    try {
      ...
      setError(err.message || 'Une erreur est survenue lors de l\'analyse du maillage.');
    } finally {
  ...
  const applyLink = async (suggestion: Suggestion) => {
    ...
    try {
      ...
      if (!targetItem) throw new Error('Cible introuvable');
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row justify-between items-end gap-6">
        <div>
          ...
          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.4em]">Optimisation de l'Architecture Sémantique & Jus SEO</p>
        </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                {isLoading && items.length === 0 ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-slate-600">
                    <p className="text-[10px] font-black uppercase tracking-widest italic">Aucune donnée trouvée</p>
                  </div>
                )}
              </div>
           </div>
           ...
           <AnimatePresence mode="wait">
             {!selectedItem ? (
               ...
               >
                 ...
                 <p className="text-xs text-slate-500 font-medium italic max-w-xs">Choisissez un contenu pour que l'IA Nexus analyse les opportunités de maillage stratégique transversales.</p>
               </motion.div>
             ) : (
               <motion.div 
                 key="analysis"
                        ...
                        <div className="flex justify-between items-start gap-6">
                           <div className="flex items-center gap-6">
                              ...
                              <div className="space-y-2">
                                 <span className="text-[9px] font-black text-blue-500 uppercase tracking-widest">Source de Mutation: {selectedItem.type}</span>
                        ...
                        {isAnalyzing ? (
                          <div className="py-20 text-center space-y-6">
                             ...
                             <div>
                                ...
                                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest animate-pulse">Exploration multi-dimensionnelle de votre base...</p>
                             </div>
                          </div>
                         ) : error ? (
                          <div className="py-12 px-8 bg-red-500/10 border border-red-500/20 rounded-3xl text-center space-y-4">
                             ...
                             <h4 className="text-lg font-black text-white uppercase italic tracking-tighter">Erreur d'analyse</h4>
                          ...
                          <div className="space-y-4">
                             <div className="flex items-center gap-3 mb-6">
                                ...
                                <h4 className="text-[10px] font-black text-white uppercase tracking-[0.2em]">{suggestions.length} Opportunités Cross-Link Détectées</h4>
                             </div>
                                         ...
                                         <div className="lg:w-48 shrink-0 flex flex-col justify-center">
                                            {isDone ? (
                                              <div className="flex flex-col items-center gap-2 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl">
                                                 ...
                                                 <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Mutation Appliquée</span>
                                              </div>
                                            ) : (
                                              <button
                                                onClick={() => applyLink(s)}
                                              ...
                                              >
                                                {isApplying === s.targetId ? (
                                                  ...
                                                  <>
                                                    Injecter le Lien <ChevronRight className="w-3 h-3" />
                                                  </>
                                                )}
                                              </button>
                                            )}
                          ...
                          <div className="py-20 text-center space-y-4">
                             ...
                             <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest italic">Aucune opportunité cross-link évidente détectée pour cette ressource.</p>
                          </div>
                        )}
                     </div>
                  </div>
