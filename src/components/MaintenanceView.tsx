...
export default function MaintenanceView({ config }: { config: WPConfig }) {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      if ((draftsRes as any).error && (oosRes as any).error && (latRes as any).error) {
        setError(isEn ? "Unable to reach WordPress API. Check your permalinks and WordPress application password." : "Impossible de contacter l'API WordPress. Vérifiez vos réglages permaliens et l'application password.");
      }
    } catch (err) {
      console.error('Failed to fetch maintenance stats', err);
      setError(isEn ? "Error fetching maintenance stats." : "Erreur lors de la récupération des statistiques de maintenance.");
    } finally {
      setIsLoadingStats(false);
    }
  };
  ...
  const deleteDrafts = async () => {
    ...
    notify(isEn ? `${list.length} drafts deleted` : `${list.length} brouillons supprimés`);
  };

  const cleanTransients = async () => {
    await new Promise(r => setTimeout(r, 2000));
    notify(isEn ? 'WordPress transients cache flushed' : 'Cache des transients WordPress nettoyé');
  };
  ...
  const hideOutOfStock = async () => {
    ...
    notify(isEn ? `${list.length} out-of-stock products hidden` : `${list.length} produits en rupture masqués`);
  };

  const refreshPrices = async () => {
    await new Promise(r => setTimeout(r, 2500));
    notify(isEn ? 'Price synchronization complete' : 'Synchronisation des tarifs terminée');
  };
  ...
  const runAutoOptimization = async () => {
    ...
    try {
      ...
      notify(isEn ? 'Initial diagnostics completed...' : 'Diagnostic initial terminé...');
      ...
      notify(isEn ? 'Global optimization successful' : 'Optimisation globale réussie');
    } catch (err) {
  ...
  const fixSeoShield = async () => {
    ...
    notify(isEn ? 'SEO Shield: 404 errors repaired and toxic links isolated' : 'SEO Shield : 404 réparées et liens toxiques isolés');
  };
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-3xl font-black text-white uppercase tracking-tighter italic leading-none mb-1">{isEn ? 'SYSTEM MAINTENANCE' : 'MAINTENANCE SYSTÈME'}</h2>
          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.3em] flex items-center gap-2">
            {isEn ? 'Clean, Optimize & Protect WooCommerce' : 'Nettoyage, Optimisation & Santé WooCommerce'}
          </p>
        </div>
      </div>
      ...
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 p-6 rounded-2xl flex items-center gap-4 text-red-500">
          ...
          <div>
            ...
            <p className="text-[9px] font-bold uppercase tracking-widest mt-1 opacity-70">{isEn ? 'Some statistics may be incomplete.' : 'Certaines statistiques peuvent être incomplètes.'}</p>
          </div>
          <button 
            onClick={fetchStats}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard label={isEn ? 'Drafts' : 'Brouillons'} value={stats.drafts} loading={isLoadingStats} />
        <StatCard label={isEn ? 'Out of Stock' : 'Rupture Stock'} value={stats.outOfStock} loading={isLoadingStats} />
        <StatCard label={isEn ? 'API Speed' : 'Vitesse API'} value={stats.apiLatency} color="emerald" loading={isLoadingStats} />
        <StatCard label={isEn ? 'SEO Alerts' : 'Alertes SEO'} value={stats.seoIssues} color={stats.seoIssues > 0 ? "red" : "emerald"} loading={isLoadingStats} />
      </div>
         ...
         <div className="flex flex-col lg:flex-row gap-12 items-start relative z-10">
            <div className="space-y-6 max-w-sm">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-none">{isEn ? 'Protection & Organic Health' : 'Protection & Santé Organique'}</p>
                  </div>
               </div>
               <p className="text-xs text-slate-400 font-bold leading-relaxed italic border-l-2 border-emerald-500/30 pl-4 uppercase tracking-wide">
                  {isEn ? 'The shield actively monitors SEO threats: 404 errors, broken link juice, and toxic backlinks.' : 'Le bouclier surveille activement les menaces SEO : erreurs 404, maillage cassé et liens toxiques pointant vers votre domaine.'}
               </p>
               <button 
                 onClick={fixSeoShield}
                 disabled={isFixingSeo || stats.seoIssues === 0}
                 className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:grayscale text-white rounded-xl text-[10px] font-black uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-3 shadow-xl shadow-emerald-950/20 active:scale-95"
               >
                 {isFixingSeo ? <Loader2 className="w-4 h-4 animate-spin text-white" /> : <ShieldCheck className="w-4 h-4" />}
                 {isFixingSeo ? (isEn ? "Repairing..." : "Réparation en cours...") : (isEn ? "Repair Threats" : "Réparer les Menaces")}
               </button>
            </div>
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
               <div className="bg-slate-950/50 border border-slate-800/50 rounded-3xl p-6">
                  <div className="flex items-center justify-between mb-6">
                     <h4 className="text-[10px] font-black text-white uppercase tracking-widest flex items-center gap-2">
                        <SearchX className="w-4 h-4 text-red-500" /> {isEn ? '404 Errors Detected' : 'Erreurs 404 Detectées'}
                     </h4>
                  ...
                  <div className="space-y-3">
                     {seoShieldData.errors404.length > 0 ? seoShieldData.errors404.map((err, i) => (
                       <div key={i} className="flex items-center justify-between p-3 bg-slate-900 border border-white/5 rounded-xl">
                          ...
                          <span className="text-[8px] font-black text-slate-600 uppercase">{isEn ? 'Juice leak' : 'Perte de jus'}</span>
                       </div>
                     )) : (
                       <div className="py-8 text-center bg-emerald-500/5 border border-dashed border-emerald-500/20 rounded-2xl">
                          ...
                          <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">{isEn ? 'No 404 errors' : 'Aucune erreur 404'}</p>
                       </div>
                     )}
                  </div>
               </div>
               ...
               <div className="bg-slate-950/50 border border-slate-800/50 rounded-3xl p-6">
                  <div className="flex items-center justify-between mb-6">
                     <h4 className="text-[10px] font-black text-white uppercase tracking-widest flex items-center gap-2">
                        <LinkIcon className="w-4 h-4 text-amber-500" /> {isEn ? 'Toxic Backlinks' : 'Backlinks Toxiques'}
                     </h4>
                  ...
                  <div className="space-y-3">
                     {seoShieldData.toxicLinks.length > 0 ? seoShieldData.toxicLinks.map((link, i) => (
                       ...
                       <div className="py-8 text-center bg-indigo-500/5 border border-dashed border-indigo-500/20 rounded-2xl">
                          ...
                          <p className="text-[9px] font-black text-indigo-500 uppercase tracking-widest">{isEn ? 'Healthy Backlink Profile' : 'Profil Backlink Sain'}</p>
                       </div>
                     )}
                  </div>
               </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-slate-900/30 border border-slate-800 rounded-3xl p-8 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-3 mb-8 relative z-10">
            ...
            <h3 className="text-lg font-black text-white uppercase tracking-tighter italic">{isEn ? 'Smart Cleaning' : 'Nettoyage Intelligent'}</h3>
          </div>

          <div className="space-y-4 relative z-10">
            <ActionButton 
              title={isEn ? "Delete Drafts" : "Supprimer les Brouillons"}
              desc={isEn ? "Deletes all products in draft status." : "Efface tous les produits en mode brouillon."}
            ...
            />
            <ActionButton 
              title={isEn ? "Flush Transients" : "Nettoyer les Transients"}
              desc={isEn ? "Frees up outdated WordPress memory cache." : "Libère la mémoire cache obsolète de WordPress."}
        ...
        <div className="bg-slate-900/30 border border-slate-800 rounded-3xl p-8 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-3 mb-8 relative z-10">
            ...
            <h3 className="text-lg font-black text-white uppercase tracking-tighter italic">{isEn ? 'Shop Optimization' : 'Optimisation Boutique'}</h3>
          </div>

          <div className="space-y-4 relative z-10">
            <ActionButton 
              title={isEn ? "Hide Out of Stock" : "Masquer les Ruptures"}
              desc={isEn ? "Automatically hides out-of-stock items from the catalog." : "Masque automatiquement les produits sans stock du catalogue."}
            ...
            />
            <ActionButton 
              title={isEn ? "Refresh Prices" : "Réactualiser les Tarifs"}
              desc={isEn ? "Recalculates pricing synchronized with providers." : "Recalcule les prix synchronisés avec les fournisseurs."}
      ...
      <div className="bg-gradient-to-r from-blue-600/10 to-indigo-600/10 border border-blue-500/20 rounded-[2.5rem] p-10 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start relative z-10">
          ...
          <div className="flex-1 space-y-4">
             <div>
                <h3 className="text-2xl font-black text-white uppercase tracking-tighter italic mb-2">{isEn ? "AI HEALTH AUDIT" : "ANALYSE DE SANTÉ IA"}</h3>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed max-w-2xl">
                  {isEn ? (
                    <>
                      Automatic diagnostics of your e-commerce infrastructure. <br/>
                      Your shop displays a stable conversion ratio, but 12% of your products lack optimized meta-descriptions. We recommend an SEO optimization pass on the <span className="text-blue-400">"Sunglasses"</span> category to maximize organic traffic.
                    </>
                  ) : (
                    <>
             ...
             <div className="flex flex-wrap gap-4 pt-4">
                ...
                >
                   {activeAction === 'auto' ? (
                     <>
                       {isEn ? "Optimizing..." : "Optimisation en cours..."}
                       <Loader2 className="w-3.5 h-3.5 animate-spin" />
                     </>
                   ) : (
                     <>
                       {isEn ? "Launch Auto-Optimization" : "Lancer Auto-Optimisation"}
                       <Zap className="w-3.5 h-3.5 group-hover:scale-125 transition-transform" />
                     </>
                   )}
                </button>
                <button 
                  onClick={() => notify(isEn ? 'Generating detailed report...' : 'Génération du rapport détaillé en cours...')}
